...
def _msgpack_path(name: str) -> str:
...
def migrate_bank():
    ...
    if not isinstance(data, dict):
        print(f"  [SKIP] bank.msgpack: not a dict")
        return
    ...
    for uid_str, acct in data.items():
        ...
        try:
            ...
            continue
        existing = bank_get(uid)
        fields = {}
        for k in ("wallet", "bank", "debt", "last_work", "last_beg", "last_daily",
                   "last_crime", "last_rob", "cf_wins", "cf_losses", "cf_net",
                   "bj_wins", "bj_losses", "bj_pushes", "bj_net"):
            if k in acct:
                fields[k] = acct[k]
        if fields:
            bank_update(uid, **fields)
    ...
    for uid_str, items in data.items():
        ...
        for entry in items:
            if isinstance(entry, dict):
                inv_add(uid, entry.get("item", ""), entry.get("name", ""))
    _backup("inventory.msgpack")
    _migrated += 1
    ...
    for uid_str, entries in data.items():
        ...
        for entry in entries:
            if isinstance(entry, dict):
                ...
                note = entry.get("note", "")
                tx_add(uid, kind, amount, note)
    _backup("tx_history.msgpack")
    _migrated += 1
...
def migrate_rps():
    ...
    for uid_str, stats in data.items():
        ...
        try:
            ...
            continue
        rps_get(uid)
        rps_update(uid, stats.get("wins", 0), stats.get("losses", 0), stats.get("ties", 0))
    _backup("rps.msgpack")
    _migrated += 1
...
def migrate_reaction_roles():
    ...
    if not isinstance(data, dict):
        return
    for key, role_id in data.items():
        rr_set(key, role_id)
    _backup("reaction_roles.msgpack")
    _migrated += 1
        ...
        for uid_str, warns in users.items():
            ...
            for w in warns:
                if isinstance(w, dict):
                    warn_add(guild_id, uid, w.get("reason", ""), w.get("mod", 0))
    _backup("warnings.msgpack")
    _migrated += 1
...
def migrate_codes():
    ...
    if not isinstance(data, dict):
        return
    for code_key, entry in data.items():
        if isinstance(entry, dict):
            codes_set(code_key, entry.get("amount", 0), entry.get("uses", 0), entry.get("created_by", 0))
    _backup("codes.msgpack")
    _migrated += 1
...
def migrate_levels():
    ...
    for uid_str, entry in data.items():
        ...
        try:
            ...
            continue
        level_get(uid)
        level_update(uid, entry.get("xp", 0), entry.get("level", 1))
    _backup("levels.msgpack")
    _migrated += 1
    ...
    for gid_str, users in data.items():
        ...
        for uid_str, count in users.items():
            ...
            for _ in range(int(count)):
                dumbass_increment(gid, uid)
    _backup("dumbass.msgpack")
    _migrated += 1
...
def main():
    ...
    print(f"\n  done: {_migrated} files migrated, {_errors} errors")
